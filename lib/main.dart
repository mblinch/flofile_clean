...
class _CaptionBuilderState extends State<CaptionBuilder>
  ...
  final double _fixedChipWidth = 200.0;
                                        ...
                                        (label) => Padding(
                                          ...
                                          child: Row(
                                            ...
                                            children: [
                                              ...
                                              // Show RBI options inline for Single, Double, Triple
  ...
  void _processScrollEvent() {
    ...
    for (int i = preloadStart; i <= preloadEnd; i++) {
      if (i < imagePaths.length) {
        // Flutter's Image.file handles thumbnail loading automatically when visible
      }
    }

    // Flutter's Image.file handles memory management automatically
  }
  ...
  void initState() {
    ...
    _typewriterController.addListener(() {
      print(
          'DEBUG: Typewriter animation listener called, progress: ${_typewriterAnimation.value}');
      if (_targetCaption.isNotEmpty && _previousCaption.isNotEmpty) {
        ...
        final newDisplayedCaption = displayWords.join(' ');
        setState(() {
          _displayedCaption = newDisplayedCaption;
        });

        // Update the caption controller text during animation
        if (captionController.text != newDisplayedCaption) {
          captionController.text = newDisplayedCaption;
        }

        // When animation completes, ensure we have the final text
        if (progress >= 1.0) {
        ...
        final newDisplayedCaption = words.take(targetWords).join(' ');
        setState(() {
          _displayedCaption = newDisplayedCaption;
        });
  ...
  void _startTypewriterEffect(String newCaption) {
    print('DEBUG: _startTypewriterEffect called with: $newCaption');
  ...
  // Preload all thumbnails using Flutter's precacheImage for persistent loading
  void _preloadThumbnails(int currentIndex) {
    if (imagePaths.isEmpty) return;

    // Preload all thumbnails into Flutter's image cache
    for (int i = 0; i < imagePaths.length; i++) {
      precacheImage(FileImage(File(imagePaths[i])), context);
    }
  }

  // Removed duplicate function - using the improved version below

  // Preload images using Flutter's precacheImage for faster loading
  void _preloadCurrentAndAdjacentImages() {
    if (imagePaths.isEmpty) return;

    // Preload current image and adjacent images for instant navigation
    final indices = <int>[
      currentIndex,
      if (currentIndex > 0) currentIndex - 1,
      if (currentIndex < imagePaths.length - 1) currentIndex + 1,
    ];

    for (final index in indices) {
      if (index >= 0 && index < imagePaths.length) {
        precacheImage(FileImage(File(imagePaths[index])), context);
      }
    }
  }
    ...
    return Container(
      ...
      child: ClipRRect(
        ...
        child: Image.file(
          ...
          width: 40, // Smaller thumbnail size
          height: 40, // Smaller thumbnail size
          ...
          return Container(
            ...
            child: ClipRRect(
              ...
              child: Image.file(
                ...
                filterQuality: FilterQuality.medium, // Better scaling quality
                errorBuilder: (context, error, stackTrace) {
                  print('Error loading thumbnail image $imagePath: $error');
                  return Container(
                    color: Colors.grey.shade200,
                    child: const Center(
                      child: Icon(
  ...
  Future<Size> _getImageDimensions(String imagePath) async {
    try {
      ...
      // Return a default size if there's any error
                          ...
                          setState(() {
                            if (isHomeList) {
                              if (isSelected) {
                                ...
                                // Check if the deselected player is the main player
                                if (code == _getFirstSelectedPlayer()) {
                                  // Reset the caption if the main player is deselected
                                  captionController.clear();
                                }
                              } else {
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: Material(
                        ...
                        child: InkWell(
                          onTap:
                              imagePaths.isNotEmpty ? () => nextImage() : null,
                          borderRadius: BorderRadius.circular(4),
                          child: Container(
                            padding: const EdgeInsets.symmetric(
                              horizontal: 8,
  ...
  void _updateCaption() {
    ...
    if (_selectedVerb == 'hit' && _selectedHitType != null) {
      ...
      // Highlight 'At Bat' verb without moving the next menu item
    ...
    if (trimmedCaption != previousCaption && trimmedCaption.isNotEmpty) {
      if (_customCelebrationVerb.isNotEmpty || wasCustomVerb) {
        ...
        // Reset manually typing flag for programmatic updates
        _isManuallyTyping = false;
  ...
  Widget build(BuildContext context) {
    ...
    return Shortcuts(
      ...
      child: Actions(
        ...
        child: GestureDetector(
          onTap: () {
            // Remove grid focus when clicking outside the grid
            setState(() {
            ...
            onKeyEvent: (KeyEvent event) {
              ...
              if (event is KeyDownEvent && imagePaths.isNotEmpty) {
                ...
                if (isInTextField) {
                  ...
                  // Not in text field: only navigate if grid is focused
                  shouldNavigate = _isThumbnailGridFocused;
                }
                            ...
                            setState(() {
                              if (isHomeList) {
                                if (isSelected) {
                                  ...
                                  if (_selectedVerb == 'Celebrate' &&
                                      !isSelected) {
                                    // Revert to default caption when 'celebrates' is deselected
                                    captionController.text = 'hits a single';
                                  }
                                } else {
                                  // In celebration mode, don't clear existing players - just add to celebration lists
                                  if (_selectedVerb == 'Celebrate' ||
