...
class _CaptionBuilderState extends State<CaptionBuilder>
            ...
            Padding(
              ...
              child: MenuAnchor(
                ...
                menuChildren: [
                  // First option: No IN
                  MenuItemButton(
                    onPressed: () => onInningSelected(null),
                    style: MenuItemButton.styleFrom(
                      minimumSize: const Size(0, 24),
                      padding: const EdgeInsets.symmetric(horizontal: 16.0),
                    ),
                    child: const Text('No IN', style: TextStyle(fontSize: 12)),
                  ),
                  const Divider(height: 1),
                  ...List.generate(9, (i) => i + 1).map((inning) {
                    return MenuItemButton(
                      onPressed: () => onInningSelected(inning),
                      style: MenuItemButton.styleFrom(
                          minimumSize: const Size(0, 24),
                          padding:
                              const EdgeInsets.symmetric(horizontal: 16.0)),
                      child: Text('$inning IN',
                  ...
                  SubmenuButton(
                    style: MenuItemButton.styleFrom(
                        ...
                        padding: const EdgeInsets.symmetric(horizontal: 16.0)),
                    menuChildren: [
                      ...List.generate(21, (i) => i + 10).map((inning) {
                        return MenuItemButton(
                          onPressed: () => onInningSelected(inning),
                          style: MenuItemButton.styleFrom(
                              minimumSize: const Size(0, 24),
                    ...
                    child: const Text('Extra Innings...',
                        style: TextStyle(
                  ...
                  if (_selectedHitType != null)
                    Padding(
                      padding: EdgeInsets.only(
                          left: _hitIndent.clamp(0.0, double.infinity),
                          top: 4.0),
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              if (_isSoloCelebration ||
                                  celebrateWith.isNotEmpty) ...[
                                // TOOLBAR ROW REPLACEMENT START (celebration)
                                const SizedBox(width: 8.0),
                                _buildArrow(),
                                const SizedBox(width: 8.0),
                                _buildActionToolbar(),
  ...
  final TextEditingController provinceController =
      TextEditingController(text: 'Ontario'); // Unified controller name
...
// Example function to parse Exiftool XML and extract fields including Personality
  ...
  Future<void> _initializeTeamData() async {
    // Load initial team data sequentially to prevent race conditions.
    // First, load the home team (which includes location data).
    await _loadTeam(selectedHomeTeam!, isHomeTeam: true);
    // After the home team is loaded, load the away team.
    // Check if the widget is still mounted before making the second call.
    if (mounted) {
  ...
  Future<void> _saveCaptionToFile(String path) async {
    ...
    await Process.run(
      ...
      [
        ...
        if (suppCat1.isNotEmpty || suppCat2.isNotEmpty || suppCat3.isNotEmpty)
          '-SupplementalCategories=${[
            suppCat1,
            suppCat2,
            suppCat3
