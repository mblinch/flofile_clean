...
class WnbaRosterFetchResult {
  WnbaRosterFetchResult({required this.players, this.headCoach});
  final List<Player> players;
  final String? headCoach;
}

/// ESPN WNBA endpoints (same unofficial public JSON used by ESPN's site; no API key).
class WnbaApiService {
  static const String _baseUrl = 'site.api.espn.com';
  static const String _basePath = '/apis/site/v2/sports/basketball/wnba';
  static const Duration _timeout = Duration(seconds: 15);

  /// Fetches all WNBA teams from ESPN.
  Future<List<TeamInfo>> fetchAllTeams() async {
    final url = Uri.https(_baseUrl, '$_basePath/teams', {'limit': '20'});
    final response = await http.get(url).timeout(
          _timeout,
          onTimeout: () => throw Exception('ESPN WNBA teams request timed out'),
        );

    if (response.statusCode != 200) {
      throw Exception('ESPN WNBA teams fetch failed: ${response.statusCode}');
    }
  ...
  Future<WnbaRosterFetchResult> fetchRosterWithHeadCoachByTeamId(
      String teamId) async {
    ...
    return WnbaRosterFetchResult(players: players, headCoach: headCoach);
  }
  ...
  Future<Map<String, dynamic>> _fetchRosterJson(String teamId) async {
    ...
    final response = await http.get(url).timeout(
          _timeout,
          onTimeout: () =>
              throw Exception('ESPN WNBA roster request timed out for team $teamId'),
        );

    if (response.statusCode != 200) {
      throw Exception('ESPN WNBA roster fetch failed: ${response.statusCode}');
    }
